<template>
  <div>
    ...
    <el-breadcrumb separator-class="el-icon-arrow-right">
      ...
      <el-breadcrumb-item>一级代理</el-breadcrumb-item>
    </el-breadcrumb>
    <!--卡片视图区域-->
    <el-card class="box-card">
      ...
      <el-table :data="userlist" style="width: 100%" border stripe>
        <el-table-column prop="id" label="一级代理ID"></el-table-column>
    ...
    >
      <el-form
        ...
        label-width="108px"
      >
        <el-form-item label="一级代理ID" prop="id">
  ...
  methods: {
    async getUserList () {
      const { data: res } = await this.$http({
        url: '/cu_admin/user/get1agentinfo.asp',
        method: 'get',
    ...
    editUserInfo () {
      this.$refs.editFormRef.validate(async (valid, field) => {
        ...
        // 发起修改用户信息的数据请求
        const { data: res } = await this.$http.post(
          '/cu_admin/user/modify1agentinfo.asp',
          this.editForm
        )
        if (res !== 'success') {
          return this.$message.error(res)
    ...
    async removeUserById (id) {
      ...
      const { data: res } = await this.$http.get(
        '/cu_admin/user/delete1agent.asp?id=' + id
      )
      if (res !== 'success') {
